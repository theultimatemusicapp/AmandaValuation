const config = {
  ...
  const formatCurrency = (value) => {
    ...
    if (Math.abs(safe) < 10) return `$${safe.toFixed(2)}`;
  ...
  button.onclick = async () => {
    try {
      ...
      const buildInputWarnings = () => {
        ...
        if (sanitizedData.ltv > 0 && sanitizedData.arr > 0 && sanitizedData.ltv < 10) {
        ...
        if (sanitizedData.runway < 3 && sanitizedData.burnRate > 0) {
          warnings.push('Runway under 3 months with burn above zero signals urgent cash needs and discounts.');
        }
        return warnings;
      };
      ...
      yPos += 10;
      doc.setFontSize(10);
        const methodsUsed = sanitizedData.valuations?.length ? sanitizedData.valuations.map(v => v.method) : sanitizedData.methods;
        doc.text(`Business: ${sanitizedData.companyName || 'Not specified'} • Type: ${sanitizedData.businessType} • Methods: ${methodsUsed.join(', ')}`, margin, yPos);
      yPos += 6;
      ...
      const midpoint = Math.round((sanitizedData.rangeLow + sanitizedData.rangeHigh) / 2);
        doc.text(`Fair market value midpoint: $${midpoint.toLocaleString()} with an ${sanitizedData.confidence}% confidence score based on completeness and method alignment.`, margin, yPos, { maxWidth: pageWidth - 2 * margin });
      yPos += 8;
      ...
      yPos += 8;
        doc.text(`Why this valuation? This valuation is primarily driven by ARR of ${formatCurrency(sanitizedData.arr)}, growth of ${formatPercent(sanitizedData.growthYoy)} YoY, churn of ${formatPercent(sanitizedData.customerChurn)}, and gross margin of ${formatPercent(sanitizedData.grossMargin)}. The applied multiple of ${sanitizedData.multiplier}× reflects ${stage} SaaS with ${profitProfile} economics.`, margin, yPos, { maxWidth: pageWidth - 2 * margin });
      yPos += 10;
      ...
      doc.autoTable({
        ...
        body: [
          ...
          [`Interpretation: Range captures sensitivity across methods (${methodsUsed.join(', ')}). Confidence dips if inputs are sparse or methods diverge.`],
      ...
      doc.setTextColor(...palette.slate);
      doc.text(`Headline valuation derived from ${methodsUsed.join(', ')} using the same applied multiple logic shown above.`, margin, yPos, { maxWidth: pageWidth - 2 * margin });
      doc.setTextColor(...palette.ink);
      yPos += 6;
      ...
      yPos = sectionSubtitle('These buyer types are inferred from your scale, growth, profitability, and market focus.', yPos);
      doc.setFontSize(10);
      ...
      yPos = sectionSubtitle('Quick wins that strengthen valuation narratives before investor outreach.', yPos);
      if (upliftActions.length >= 3) {
        doc.autoTable({
          ...
          styles: { fillColor: [255, 255, 255], textColor: palette.ink, lineColor: palette.soft, cellPadding: 3 }
        });
        yPos = doc.lastAutoTable.finalY + 6;
      } else if (upliftActions.length) {
        upliftActions.forEach(item => {
          doc.text(`• ${item.area}: ${item.action} (${item.impact})`, margin, yPos, { maxWidth: pageWidth - 2 * margin });
          yPos += 6;
        });
      } else {
        doc.text('No major uplift actions detected for this data set. Maintain trajectory and refresh inputs monthly.', margin, yPos, { maxWidth: pageWidth - 2 * margin });
        yPos += 6;
      }
      yPos += 2;
      doc.setTextColor(...palette.slate);
      ...
      yPos = sectionSubtitle('Blend of approaches to triangulate a defensible range.', yPos);
        const valuationRows = sanitizedData.valuations.map(v => {
          ...
          const insights = {
            ...
            'Earnings-Based': `Balances ARR and retention (${formatPercent(sanitizedData.retentionRate)}); higher NPS (${sanitizedData.nps}) improves confidence.`,
            DCF: `Discounts future cash at ${formatPercent(sanitizedData.discountRate * 100)}; runway ${sanitizedData.runway}m and debt ${formatCurrency(sanitizedData.debtLevel)} are key.`
          };
        ...
        doc.autoTable({
          ...
          headStyles: { fillColor: palette.brand, textColor: [255, 255, 255] },
        styles: { fillColor: [255, 255, 255], textColor: palette.ink, lineColor: palette.soft, cellPadding: 3 },
        columnStyles: { 0: { cellWidth: 45 }, 1: { cellWidth: 40 }, 2: { cellWidth: pageWidth - 2 * margin - 90 } }
        });
      ...
      yPos = sectionTitle('Input Summary', yPos);
        const inputs = [
          ['Valuation Methods', methodsUsed.join(', ')],
      ...
      // Each metric now includes an "importance" explanation to show why it matters.
        const metrics = [
          ...
          insight: sanitizedData.ltv === 0 ? 'Enter a realistic LTV to strengthen credibility.' : sanitizedData.ltv < 10 ? 'Low lifetime value; focus on pricing and retention.' : sanitizedData.ltv > 3 * sanitizedData.cac ? 'Strong lifetime value supports valuation.' : 'Healthy lifetime value—continue improving retention.'
          },
          {
            label: 'CAC',
          value: formatCurrency(sanitizedData.cac),
          importance: 'Reveals cost to acquire customers.',
          insight: sanitizedData.cac === 0 && sanitizedData.arr > 0 ? 'CAC unreported; track acquisition costs for credibility.' : sanitizedData.cac < sanitizedData.ltv / 3 ? 'Efficient acquisition.' : 'Lower acquisition costs.'
          },
          {
            label: 'Gross Margin',
      ...
      doc.autoTable({
        ...
        styles: { fillColor: palette.soft, textColor: palette.ink, cellPadding: 3 },
        headStyles: { fillColor: palette.brand, textColor: [255, 255, 255] },
        columnStyles: { 0: { cellWidth: 32 }, 1: { cellWidth: 35 }, 3: { cellWidth: pageWidth - 2 * margin - 74 } }
      });
      yPos = doc.lastAutoTable.finalY + 8;
      doc.setFontSize(10);
      doc.setTextColor(...palette.slate);
      ...
      yPos = addPage('Methodology');
        ...
        yPos = sectionSubtitle('Shows which approaches informed this valuation and how to interpret them.', yPos);
        const methodology = [
          ['Revenue Multiplier', `ARR ${formatCurrency(sanitizedData.arr)} × ${sanitizedData.multiplier}x`, 'Revenue Multiple Market Context (for reference).'],
          ...
          ['DCF', `${formatPercent(sanitizedData.discountRate * 100)}`, 'Projects cash flows and discounts for risk, runway, and leverage.']
        ];
        const methodologyUsed = valuationRows.length ? valuationRows.map(v => v[0]) : methodsUsed;
        doc.autoTable({
          head: [['Approach', 'Formula / Key Inputs', 'How to Read']],
          body: methodology.filter(row => methodologyUsed.includes(row[0])),
          startY: yPos,
          theme: 'grid',
          headStyles: { fillColor: palette.ink, textColor: [255, 255, 255] },
          styles: { fillColor: [255, 255, 255], textColor: palette.ink, lineColor: palette.soft, cellPadding: 3 }
      ...
      yPos = addPage('Market & Confidence');
        ...
        yPos = sectionSubtitle('Revenue Multiple Market Context (for reference).', yPos);
      ...
      yPos += 10;
        ...
        yPos = sectionSubtitle('Breakdown of the overall confidence score across key dimensions.', yPos);
        doc.autoTable({
